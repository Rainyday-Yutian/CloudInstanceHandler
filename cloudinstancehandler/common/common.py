...
class BasicDataFrame():
    def __init__(self)-> None:
        ...
        self.Prefix = "CIDH"  # Cloud Instance Data Handler / Cloud Instance Handler
        # 后期更新改成CSP_Prefix，标识清楚变量用于方便区分不同云服务商的模块
        self.DataDir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data')
    ...
    def statisticMetricData(self,field_name:str,df_data:pd.DataFrame=None,statistics_approach:list=['max'],group_by:str=None,rename_field_name:str=None)-> pd.DataFrame:
        ...
        if group_by and group_by not in df_data.columns:
            raise ValueError(f"group_by column '{group_by}' not found in DataFrame columns.")
        # group_by功能需要改进，group_by改为列表，允许多个维度
    ...
    def _save_dataframe(self,df:pd.DataFrame,path:str=None,rename_suffix:str=None,format:str="xlsx",split_by:str=None)-> None:
        ...
        """
        output_dir = path if path is not None else self.DataDir
        try:
        ...
        if split_by is not None and split_by not in df.columns:
            ...
            split_by = None
        file_path = os.path.join(output_dir, f'{self.Prefix}_{self.InsType}')
        if rename_suffix:
        ...
        try:
            ...
            file_path = os.path.join(self.DataDir, f'{self.Prefix}_{self.InsType}_Info.csv')
